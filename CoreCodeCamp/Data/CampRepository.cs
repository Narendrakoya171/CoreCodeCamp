   ...
   public class CampRepository: ICampRepository
    {
        ...
        private readonly ILogger<CampRepository> _ilogger;

         public CampRepository(CampContext context, ILogger<CampRepository> ilogger)
        {
            _context = context;
            _ilogger = ilogger;
        ...
        {
            _ilogger.LogInformation($"Adding an object of type {entity.GetType()} to the context.");
            _context.Add(entity);
        }

        public void Delete<T>(T entity) where T : class
        {
            _ilogger.LogInformation($"Removing an object of type {entity.GetType()} to the context.");
            _context.Remove(entity);
        }
        public async Task<bool> SaveChangesAsync()
        {
            _ilogger.LogInformation($"Attempitng to save the changes in the context");
        ...
        {
            _ilogger.LogInformation($"Getting all Camps");
        ...
        {
            _ilogger.LogInformation($"Getting a Camp for {moniker}");
        ...
        {
            _ilogger.LogInformation($"Getting Speaker");
        ...
        {
            _ilogger.LogInformation($"Get all speakers by moniker");
        ...
        {
            _ilogger.LogInformation($"Get Talk by moniker by camp");
        ...
        {
            _ilogger.LogInformation($"Get all talks by camp");

            IQueryable<Talk> query = _context.Talks;
            if (includeSpeakers)
            {
                query = query.Include(s => s.Speaker);
